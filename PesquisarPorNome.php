...
//var_dump($_GET);
     ...
     */
    $LIMITE = 10;
    $SQL_COUNT = $CONEXAO->query( "SELECT id FROM $base.$tabela_pesquisa WHERE nome_analista like '%$nome_analista%'");
    $SQL_RESUL = ceil($SQL_COUNT->num_rows / $LIMITE);
    $pg = (isset($_GET["pg"])) ? (int)$_GET["pg"] : 1 ;
    $START = ($pg - 1) * $LIMITE;
    $resultado = $CONEXAO->query( "SELECT * FROM $base.$tabela_pesquisa WHERE nome_analista like '%$nome_analista%' LIMIT $START, $LIMITE"  );
    ...
    echo "    <li>";
    echo "      <a href='?pg=1&nome=$nome_analista' aria-label='Previous'>";
                ...
                if ($SQL_RESUL > 1 && $pg<=$SQL_RESUL){
                   for ( $i=$inicio; $i<=$fim; $i++ ){
                       ...
                       if ($i == $pg){
                            ...
                            echo "<li><a href='?pg=$i&nome=$nome_analista'>$i</a></li>";
                       }
                   }        
                }
    echo "    <li>";
    echo "      <a href='?pg=$SQL_RESUL&nome=$nome_analista'' aria-label='Next'>";
